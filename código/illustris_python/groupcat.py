...
def loadMany(basePath, snapNum, subhaloIDs=[], fields=[]):
    """ Return complete group catalog information for one halo or subhalo. """
    if len(subhaloIDs) == 0:
        raise Exception("Must specify at least one ID in subhaloIDs.")

    if len(fields) == 0:
        raise Exception("Must specify at least one field in fields.")

    subhaloIDs = np.sort(subhaloIDs)

    with h5py.File(gcPath(basePath, snapNum), 'r') as f:
        offsets = np.append(f['Header'].attrs['FileOffsets_Subhalo'],
                            f['Header'].attrs['Nsubgroups_Total'])

    ids_by_file = []

    for left, right in zip(offsets, offsets[1:]):
        mask = (left <= subhaloIDs) & (subhaloIDs < right)
        ids_by_file.append(subhaloIDs[mask])

    # load halo/subhalo fields into a dict
    result = {field: [] for field in ['SubfindID'] + fields}

    for fileNum, ids in enumerate(ids_by_file):
        if len(ids) == 0: continue

        with h5py.File(gcPath(basePath, snapNum, fileNum), 'r') as f:
            result['SubfindID'].append(ids)

            offset_ids = ids - offsets[fileNum]

            for subhaloProp in fields:
                result[subhaloProp].append(f['Subhalo'][subhaloProp][list(offset_ids)])
